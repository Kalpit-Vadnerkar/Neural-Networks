...
class Model(object):
    def __init__(self, learning_rate):
        ...
        self.param = {
           ...
           "w3":nn.parameter.from_numpy(numpy.random.normal(0, 0.01,[250, 100])),
           "b3":nn.parameter.zeros([100]),
           "w4":nn.parameter.from_numpy(numpy.random.normal(0, 0.01,[100, n_classes])),
    ...
    def get_loss(self, x, y):
        ...
        probs = None
        loss  = None
        loss, probs = nn.softmax_cross_entropy_loss(logits = model.forward_propagation(x), labels = y, axis=-1)
        #######################################################################
    ...
    try:
        ...
        for epoch in range(1, MAX_EPOCHS+1):
            ...
            for step, (x, y) in enumerate(DB.train, 1):            
                ...
                global_step += 1
                logger.log(loss.item(), accuracy, global_step, not global_step%100)
                #######################################################################
